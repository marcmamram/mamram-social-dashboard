...
1. **The Meta permission lapses on 2026-10-12.** Meta expires an app's access
   ...
   **Who must do it:** the token has to be generated by someone with a role on
   the Facebook Page *and* on the Meta app "Mamram Social". As of July 2026
   that app has exactly one administrator, **Bar Turgeman** (the social media
   manager). Pasting the result into the repository secret needs the office
   GitHub login. So renewal is a two-person job unless one person holds both.
